function [ State_mat,State_des,State_veh,Control_mat,t ] = PID(gains)
...
for t=0:dt:time_end
    ...
    EKF_acc_des=x2_dot+[rel_statex(3,1);rel_statey(3,1);rel_statez(3,1)];
        ...
        if (abs(rel_statex(1,1))<0.5*(abs(rel_statez(1,1))+0.01) && abs(rel_statey(1,1))<0.5*(abs(rel_statez(1,1))+0.01))
    ...
    pos_des(3,:)=height;
    %     pos_des=EKF_pos_des;
    error_outer=pos_des+0*noise*randn(3,1)-x1-0*noise*randn(3,1);
    
    error_dot_outer=(error_outer-prev_error_outer)/dt;
    error_int_outer=error_int_outer+error_outer*dt;
    u_outer=kp_outer*(error_outer)+kd_outer*(error_dot_outer)+ki_outer*(error_int_outer);
    prev_error_outer=error_outer;
    Thrust=((u_outer(1,1)^2)+(u_outer(2,1)^2)+((m*gravity(3,1)+u_outer(3,1))^2))^0.5;
    psi_des=atan2(error_outer(2,1),error_outer(1,1));
